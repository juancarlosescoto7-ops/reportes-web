...
test("encuentra acciones profundas y conserva la instrucción en la ruta", () => {
  ...
  const beneficiario = buscarEnIndiceUniversal(indice, "Nuevo beneficiario")[0];
  assert.equal(beneficiario.categoria, "accion");
  assert.equal(beneficiario.titulo, "Nuevo beneficiario");
});

test("prioriza acciones explícitas sobre registros con el mismo contexto", () => {
  const navegacion = construirIndiceNavegacion({
    permisos: ["VER_EGRESOS"],
  });
  const datos = construirIndiceBusquedaUniversal(fuentes);
  const [resultado] = buscarEnIndiceUniversal(
    [...navegacion, ...datos],
    "crear beneficiario"
  );

  assert.equal(resultado.id, "navegacion:nuevo-beneficiario");
  assert.equal(resultado.categoria, "accion");
});
