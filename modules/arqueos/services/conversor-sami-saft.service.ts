...
function crearErrorCatalogo(
  ...
  if (tablaNoInstalada) {
    return new Error(
      `No se encontró la tabla ${tabla} en Supabase. Ejecute modules/arqueos/database/arqueos_conversion_sami.sql antes de usar el conversor.`
    );
  }
...
async function obtenerDetalleErrorGuardarEquivalencia(response: Response) {
  ...
  if (data && typeof data === "object") {
    ...
    if (detalle.code === "PGRST202") {
      return "La función para guardar equivalencias aún no está instalada en Supabase. Ejecute modules/arqueos/database/guardar_equivalencia_rubro_ingreso.sql y vuelva a intentarlo.";
    }
