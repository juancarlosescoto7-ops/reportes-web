...
export default function BorradorGastosWorkspace({ data, onChanged }: { data: RespuestaBorrador; onChanged: () => Promise<void> }) {
  ...
  return (
    <div className="relative flex h-full min-h-[640px] flex-col overflow-hidden">
      ...
      <button data-shortcut="203" type="button" onClick={() => setFormOpen(true)} className="absolute bottom-5 right-5 z-20 inline-flex h-11 items-center gap-2 rounded-full bg-[#005f48] px-5 text-sm font-semibold text-white shadow-lg shadow-emerald-950/20">
    ...
    <>
      <div className="grid min-h-12 grid-cols-[1fr_180px] items-center border-b border-slate-100 px-3 hover:bg-emerald-50/30" style={{ paddingLeft: 12 + depth * 18 }}>
        <div className="flex min-w-0 items-center gap-2">
          {node.codigo ? <span className="h-1.5 w-1.5 shrink-0 bg-emerald-500" /> : <button data-shortcut="204" type="button" onClick={() => setOpen((current) => !current)} className="grid h-7 w-7 shrink-0 place-items-center text-slate-500" aria-label={open ? "Contraer nivel" : "Expandir nivel"}>{open ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}</button>}
        ...
        <div className="flex justify-end gap-1">
          {node.codigo ? <><input type="number" min="0" step="0.01" value={value} onChange={(event) => setValue(event.target.value)} className="h-8 w-28 border border-slate-300 px-2 text-right text-xs" aria-label={`Monto ${node.code}`} /><button data-shortcut="205" type="button" onClick={() => void save()} disabled={busy} className="grid h-8 w-8 place-items-center bg-[#005f48] text-white disabled:opacity-40" title="Guardar monto"><Save className="h-3.5 w-3.5" /></button><button data-shortcut="206" type="button" onClick={() => void remove()} disabled={busy} className="grid h-8 w-8 place-items-center border border-rose-200 text-rose-600 disabled:opacity-40" title="Retirar código"><Trash2 className="h-3.5 w-3.5" /></button></> : <span className="text-xs font-semibold tabular-nums text-slate-800">{money(node.amount)}</span>}
        </div>
      </div>
