...
test("abre Pendientes primero para quien tiene acceso al dashboard", () => {
  assert.equal(
    obtenerRutaInicialSesion({
      rolCodigo: "ADMIN",
      rutasConfiguradas: ["/"],
      rutasAdicionales: ["/pendientes"],
    }),
    "/pendientes"
  );
});

test("abre el reporte de Oficina de la Mujer cuando es la unica ruta adicional", () => {
  assert.equal(
    obtenerRutaInicialSesion({
      rolCodigo: "OFICINA_MUJER",
      rutasConfiguradas: [],
      rutasAdicionales: ["/reportes/oficina-mujer"],
    }),
    "/reportes/oficina-mujer"
  );
});
