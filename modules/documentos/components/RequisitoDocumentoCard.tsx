...
import { DocumentoProyecto } from "@/modules/documentos/services/documentacionProyectos";
import { subirDocumentoProyecto } from "@/modules/documentos/services/documentosProyecto.service";
                    ...
                    <div className="pointer-events-none absolute right-2 top-2 h-32 w-32 overflow-hidden rounded-full border-2 border-white bg-black shadow-2xl">
                      <div
                        ...
                        style={{
                          ...
                          backgroundSize: `${(tamanoEditor?.width ?? 0) * ZOOM_LUPA}px ${(tamanoEditor?.height ?? 0) * ZOOM_LUPA}px`,
                          backgroundPosition: calcularFondoLupa(
...
function calcularFondoLupa(
  x: number,
  y: number,
  anchoCanvas: number,
  altoCanvas: number,
  tamanoEditor: TamanoEditor | null
) {
  if (!tamanoEditor) return "50% 50%";

  const xEditor = (x / anchoCanvas) * tamanoEditor.width;
  const yEditor = (y / altoCanvas) * tamanoEditor.height;
  const xFondo = TAMANO_LUPA / 2 - xEditor * ZOOM_LUPA;
  const yFondo = TAMANO_LUPA / 2 - yEditor * ZOOM_LUPA;

  return `${xFondo}px ${yFondo}px`;
}
